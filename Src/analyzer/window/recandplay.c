...
static char g_rec_etykiety_slotow[14][32];

static void REC_WypelnijKontrolki(UI_KONTROLKA_t kontrolki[recMenus_Length])
{
    static const uint16_t slot_y[7] = { 3U, 36U, 69U, 100U, 131U, 162U, 193U };
    static const uint16_t slot_h[7] = { 33U, 32U, 31U, 31U, 31U, 31U, 31U };
    uint8_t i;

    kontrolki[MENU_EXIT] = UI_UtworzKontrolke(MENU_EXIT, 0, 237, 80, 35,
        "Wstecz", UI_STYL_POWROT, UI_ROLA_TEKSTU_PRZYCISK, true, false);
    kontrolki[MENU_SAVECONFIG] = UI_UtworzKontrolke(MENU_SAVECONFIG, 80, 237, 100, 35,
        "Zapisz", UI_STYL_NORMALNY, UI_ROLA_TEKSTU_PRZYCISK, true, false);
    kontrolki[MENU_SETPTT] = UI_UtworzKontrolke(MENU_SETPTT, 180, 237, 100, 35,
        "Sterowanie PTT", UI_STYL_NORMALNY, UI_ROLA_TEKSTU_PRZYCISK, true, *pttControl != 0U);
    kontrolki[MENU_PLAY] = UI_UtworzKontrolke(MENU_PLAY, 280, 237, 100, 35,
        (*RecPlayStatus == AUDIO_ST_NONE) ? "Odtworz" : "STOP",
        (*RecPlayStatus == AUDIO_ST_NONE) ? UI_STYL_AKCENT : UI_STYL_OSTRZEZENIE,
        UI_ROLA_TEKSTU_PRZYCISK, true, *RecPlayStatus == AUDIO_ST_PLAY);
    kontrolki[MENU_RECORD] = UI_UtworzKontrolke(MENU_RECORD, 380, 237, 100, 35,
        (*RecPlayStatus == AUDIO_ST_NONE) ? "Nagraj" : "STOP",
        (*RecPlayStatus == AUDIO_ST_NONE) ? UI_STYL_AKCENT : UI_STYL_OSTRZEZENIE,
        UI_ROLA_TEKSTU_PRZYCISK, true, *RecPlayStatus == AUDIO_ST_REC);

    /* Historyczne pozycje 5..8 mialy zerowa szerokosc i nie byly realnymi przyciskami. */
    for (i = MENU_BANDDOWN; i <= MENU_AFFREQUP; ++i)
        kontrolki[i] = UI_UtworzKontrolke(i, 0, 0, 0, 0, "", UI_STYL_NIEAKTYWNY,
                                           UI_ROLA_TEKSTU_PRZYCISK, false, false);

    for (i = 0U; i < 14U; ++i)
    {
        uint8_t rzad = (uint8_t)(i / 2U);
        uint8_t kolumna = (uint8_t)(i % 2U);
        uint8_t indeks_pliku = (uint8_t)(i + 1U);
        uint16_t x = kolumna == 0U ? 280U : 379U;
        uint16_t szerokosc = kolumna == 0U ? 100U : 101U;

        if (sndFileSizes[indeks_pliku - 1U] != 0U)
        {
            char czas[16];
            GetTimeStrByFileIndex(indeks_pliku, czas);
            snprintf(g_rec_etykiety_slotow[i], sizeof(g_rec_etykiety_slotow[i]),
                     "[%2u] %s", (unsigned)indeks_pliku, czas);
        }
        else
        {
            snprintf(g_rec_etykiety_slotow[i], sizeof(g_rec_etykiety_slotow[i]),
                     "[%2u] pusto", (unsigned)indeks_pliku);
        }

        kontrolki[MENU_SLOT1 + i] = UI_UtworzKontrolke((int16_t)(MENU_SLOT1 + i),
            x, slot_y[rzad], szerokosc, slot_h[rzad], g_rec_etykiety_slotow[i],
            UI_STYL_NORMALNY, UI_ROLA_TEKSTU_PRZYCISK, true,
            *selectedSlotIndex == indeks_pliku);
    }
}

void recMenuDraw(void)
{
    UI_KONTROLKA_t kontrolki[recMenus_Length];
    REC_WypelnijKontrolki(kontrolki);
    UI_RysujKontrolki(kontrolki, recMenus_Length);
...
int DrawRateAndCheckStop(int progressPercent, uint32_t progressColor, int playingTime)
{
    ...
    if (TOUCH_Poll(&pt))
    {
        UI_KONTROLKA_t kontrolki[recMenus_Length];
        REC_WypelnijKontrolki(kontrolki);
        if (UI_CzyPunktWObszarze(pt, &kontrolki[MENU_PLAY].obszar) ||
            UI_CzyPunktWObszarze(pt, &kontrolki[MENU_RECORD].obszar))
        {
            TRACK_Beep(1);
            (*playUserStop) = 1;
            return 1;
...
#define VERSION_FONT_COLOR LCD_RGB(59, 87, 100)
    ...
    for (;;)
    {
        if (TOUCH_Poll(&pt))
        {
            ...
            UI_KONTROLKA_t kontrolki[recMenus_Length];
            REC_WypelnijKontrolki(kontrolki);
            int touchIndex = UI_ZnajdzKontrolke(pt, kontrolki, recMenus_Length);
