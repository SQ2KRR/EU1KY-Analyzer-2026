...
static void _na_male_litery(char *tekst)
{
    if (tekst == NULL)
        return;
    ...
    while (*tekst != '\0')
    {
        *tekst = (char)tolower((unsigned char)*tekst);
        tekst++;
    }
}
...
void PROTOCOL_Handler_LINSMITH(void)
{
    ...
    // Command received
    rxstr_p = _trim(rxstr);
    _na_male_litery(rxstr_p); // protokol nie rozroznia wielkosci liter
...
void PROTOCOL_Handler(void)
{
    ...
    case CFG_PROTO_NANOVNA:
        ...
        break;
    case CFG_PROTO_MINIVNA:
        PROTOCOL_Handler_VNA();
        break;
    default:
