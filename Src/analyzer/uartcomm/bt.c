...
#define BT_SSID_MAX 21U
#define BT_PIN_OFFSET 22U

static char g_bt_data[29] = {0};
static char *BT_SSID = &g_bt_data[0];
static uint32_t BT_PIN = 0U;

#include "ff.h"
#include "crash.h"
static const char *g_bt_fpath = "/aa/bt.bin";
static char *NewSSID = &g_bt_data[0];
...
void BT_StoredInformation(void)
{
    FRESULT res;
    FIL fo = {0};
    res = f_open(&fo, g_bt_fpath, FA_CREATE_ALWAYS | FA_WRITE);
    if (FR_OK == res)
    {
        UINT bw;
        memcpy(&g_bt_data[BT_PIN_OFFSET], &BT_PIN, sizeof(BT_PIN));
...
void BT_LoadInformation(void)
{
    ...
    if (FR_OK == res)
    {
        ...
        g_bt_data[BT_SSID_MAX] = '\0';
        if (br >= (BT_PIN_OFFSET + sizeof(BT_PIN)))
            memcpy(&BT_PIN, &g_bt_data[BT_PIN_OFFSET], sizeof(BT_PIN));
    }
...
void BT_ChangePW(void)
{
    ...
    changepw = NumKeypad(changepw, 0, 9999, JEZYK_Wybierz("Hasło Bluetooth", "Bluetooth password", "Bluetooth-Passwort", "Пароль Bluetooth"));
    BT_PIN = changepw;
    BT_StoredInformation();
    rqExit = 1;
}
...
void BT_ChangeSID(void)
{
    while (TOUCH_IsPressed())
        ; // wait for release
    LCD_FillAll(LCD_BLACK);
    KeyboardWindow(NewSSID, BT_SSID_MAX, JEZYK_Wybierz("Nazwa Bluetooth (SSID)", "Change Bluetooth SSID", "Bluetooth-SSID ändern", "Изменить SSID Bluetooth"));
    g_bt_data[BT_SSID_MAX] = '\0';
    BT_StoredInformation();
    rqExit = 1;
}
...
{
    char str1[128];
    const char *odpowiedz_nazwa = BT_Command("AT+NAME?");
    if (odpowiedz_nazwa != NULL && strcmp(odpowiedz_nazwa, "ERROR") != 0)
    {
        strncpy(g_bt_data, odpowiedz_nazwa, BT_SSID_MAX);
        g_bt_data[BT_SSID_MAX] = '\0';
    }
...
static const TEXTBOX_t BT_menu[] = {
    ...
    (TEXTBOX_t){.x0 = COL1, .y0 = 110, .text = " Set SSID ", .font = FONT_FRANBIG, .width = 200, .height = 34, .center = 1, .border = 1, .fgcolor = M_FGCOLOR, .bgcolor = M_BGCOLOR, .cb = BT_ChangeSID, .cbparam = 1, .next = (void *)&BT_menu[1]},
    ...
    (TEXTBOX_t){.x0 = COL1, .y0 = 160, .text = " Set Password  ", .font = FONT_FRANBIG, .width = 200, .height = 34, .center = 1, .border = 1, .fgcolor = M_FGCOLOR, .bgcolor = M_BGCOLOR, .cb = BT_ChangePW, .cbparam = 1, .next = (void *)&BT_menu[2]},
    (TEXTBOX_t){.x0 = COL2, .y0 = 160, .text = " Set Baud ", .font = FONT_FRANBIG, .width = 200, .height = 34, .center = 1, .border = 1, .fgcolor = M_FGCOLOR, .bgcolor = M_BGCOLOR, .cb = BT_ChangePW, .cbparam = 1, .next = (void *)&BT_menu[3]},
    ...
    (TEXTBOX_t){
        ...
        .y0 = 210,
        .text = " Menu główne ",
        .tekst_id = TEXTBOX_TEKST(TEKST_MENU_GLOWNE),
        .font = FONT_FRANBIG,
        .width = 200,
        .height = 34,
        .center = 1,
