 ...
 *   kd8cec@gmail.com
 *
 *   miniVNA/VNA-J compatibility path; still marked experimental until hardware verification.
...
static uint32_t MINIVNA_OgraniczADC(float wartosc)
{
    if (!isfinite(wartosc) || wartosc <= 0.0f)
        return 0U;
    if (wartosc >= 4095.0f)
        return 4095U;
    return (uint32_t)wartosc;
}
/*
...
int VnaComMode; /* 0: odbicie, 1: transmisja */
unsigned long int StartFreq;
unsigned int NumberF;
unsigned long int StepF;

/*
 * Oryginalny eksperymentalny dekoder zakładał, że cały pakiet miniVNA jest
 * już w FIFO w chwili pierwszego wywołania. Przy realnym UART/Bluetooth
 * pakiet przychodzi porcjami, więc parser tracił rozpoczęte polecenie.
 * Buforujemy ramkę do czwartego CR i dopiero wtedy ją interpretujemy.
 */
static char minivna_rx[RXB_SIZE + 1U];
static uint32_t minivna_rx_len = 0U;
static uint8_t minivna_cr = 0U;

void MINIVNA_Reset(void)
{
    minivna_rx_len = 0U;
    minivna_cr = 0U;
    minivna_rx[0] = '\0';
...
static int MINIVNA_ParsujU32(const char **p, uint32_t *wynik)
{
    uint64_t wartosc = 0U;
    uint8_t cyfry = 0U;

    if (p == NULL || *p == NULL || wynik == NULL)
        return 0;

    while (**p >= '0' && **p <= '9')
    {
        wartosc = wartosc * 10U + (uint64_t)(**p - '0');
        if (wartosc > UINT32_MAX)
            return 0;
        (*p)++;
        cyfry++;
    }
    if (cyfry == 0U || **p != '\r')
        return 0;
    (*p)++;
    *wynik = (uint32_t)wartosc;
    return 1;
}

static int MINIVNA_ParsujRamke(void)
{
    const char *p = minivna_rx;
    uint32_t start = 0U;
    uint32_t liczba = 0U;
    uint32_t krok = 0U;
    ...
    if (!MINIVNA_ParsujU32(&p, &start) ||
        !MINIVNA_ParsujU32(&p, &liczba) ||
        !MINIVNA_ParsujU32(&p, &krok))
        return 0;

    if (*p != '\0' || liczba == 0U || liczba > 4096U)
        return 0;

    StartFreq = start;
    NumberF = (unsigned int)liczba;
    StepF = krok;
    return 1;
}
...
char DecodeMiniVNA(void)
{
    ...
    while ((ch = AAUART_Getchar()) != EOF)
    {
        ...
        if (minivna_rx_len >= RXB_SIZE)
        {
            MINIVNA_Reset();
            continue;
        }

        minivna_rx[minivna_rx_len++] = (char)ch;
        minivna_rx[minivna_rx_len] = '\0';
        ...
        if (ch == '\r')
        {
            ...
            if (minivna_cr >= 4U)
            {
                const int ok = MINIVNA_ParsujRamke();
                MINIVNA_Reset();
                return ok ? 1 : 0;
            }
        }
...
void PROTOCOL_Handler_VNA(void)
{
    ...
    for (int i = 0; i < NumberF; i++)
    {
        ...
        if (!GEN_CzyCzestotliwoscObslugiwana(chkFreq) ||
            chkFreq < CFG_GetParam(CFG_PARAM_BAND_FMIN) ||
            chkFreq > CFG_GetParam(CFG_PARAM_BAND_FMAX))
        {
            mag1 = 0;
            phase1 = 0;
        ...
        /* Najpierw ograniczamy wartość zmiennoprzecinkową, a dopiero potem
           zamieniamy ją na typ bez znaku. Dzięki temu ujemna faza nie może
           zawinąć się do bardzo dużej liczby. */
        adcamp = MINIVNA_OgraniczADC((mag1 + floatToADCOffset) * floatToADCScale);
        adcphs = MINIVNA_OgraniczADC((phase1 + floatToADCOffset) * floatToADCScale);
