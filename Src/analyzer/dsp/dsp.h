...
extern "C"
{
    ...
    typedef struct
    {
        ...
        uint8_t liczba_pomiarow;
        uint8_t korekcja_hw_zadana;
        uint8_t korekcja_osl_zadana;
        uint8_t harmoniczna_generatora;
        uint8_t generator_obslugiwany;
        uint32_t czestotliwosc_bazowa_hz;
        int poprawny;
        float napiecie_v_mv;
        float napiecie_i_mv;
        ...
        DSP_RX impedancja_przed_osl;
        DSP_RX impedancja_po_osl_przed_portext;
        DSP_RX impedancja_po_osl;
        uint32_t port_extension_ps;
        uint8_t port_extension_aktywna;
    } DSP_DANE_METROLOGICZNE_t;

    void DSP_Init(void);

    void DSP_Measure(uint32_t freqHz, int applyErrCorr, int applyOSL, int nMeasurements);

    /* Nakłada skonfigurowaną kompensację przewodu na ostatni wynik po OSL. */
