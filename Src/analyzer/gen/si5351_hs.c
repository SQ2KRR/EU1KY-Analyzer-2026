...
{
    /*
     * Warstwa szybka musi używać tego samego, rzeczywiście wykrytego adresu
     * co zwykły sterownik Si5351. W trybie Auto konfiguracja zawiera 0x00,
     * więc użycie surowej wartości CFG kierowałoby ramki pod błędny adres.
     * Jeżeli układu nie wykryto, nie uruchamiamy niepotrzebnego timeoutu I2C.
     */
    if (!si5351_IsPresent())
        return 1U;

    CAMERA_IO_Write(si5351_GetBusAddress(), addr, data);
    return 0U;
}
...
uint8_t SI5351_HS_WriteN(uint8_t addr, uint8_t bytes, uint8_t *data)
{
    ...
    CAMERA_IO_WriteBulk(si5351_GetBusAddress(), addr, data, (uint16_t)bytes);
    return 0U;
}
