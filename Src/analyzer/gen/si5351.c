...
static uint8_t si5351_address_responds(uint8_t addr);
...
static uint8_t si5351_bus_addr_runtime = 0xC0;
static uint8_t si5351_obecny = 0;
...
void si5351_Init(void)
{
    ...
    configured_addr = (uint8_t)CFG_GetParam(CFG_PARAM_SI5351_BUS_BASE_ADDR);
    if (configured_addr == 0)
    {
        ...
        {
            si5351_obecny = 0;
            si5351_bus_addr_runtime = 0xC0;
            FONT_Write(FONT_FRAN, LCD_RED, LCD_BLACK, 5, 10,
                       "Si5351: autodetekcja nieudana. Sprawdź I2C.");
        }
        else
        {
            si5351_obecny = 1;
            si5351_bus_addr_runtime = addr;
            /*
             * Poprawny wynik autodetekcji jest stanem normalnym, więc nie
             * zaśmiecamy nim ekranu startowego ani późniejszych okien, które
             * ponownie inicjalizują generator. Adres pozostaje dostępny w
             * Diagnostyce i ustawieniach Si5351.
             */
        }

        /*
         * AUTO pozostaje ustawieniem AUTO. Nie zapisujemy znalezionego adresu
         * do konfiguracji, dlatego wymiana modulu nie wymaga kasowania config.bin.
         * Zwłokę zostawiamy tylko dla błędu, aby użytkownik zdążył go zobaczyć.
         */
        if (!si5351_obecny)
            Sleep(1200);
    }
    ...
    {
        si5351_bus_addr_runtime = configured_addr;
        si5351_obecny = si5351_address_responds(configured_addr);
        if (!si5351_obecny)
        {
            FONT_Print(FONT_FRAN, LCD_RED, LCD_BLACK, 5, 10,
                       "Si5351: brak odpowiedzi pod adresem %02Xh.", configured_addr);
            Sleep(1200);
        }
    }

    /*
     * Brak generatora nie moze blokowac uruchomienia interfejsu i diagnostyki.
     * Nie wysylamy dalszych transmisji I2C do nieobecnego ukladu, bo kazda z nich
     * moze czekac na timeout magistrali.
     */
    if (!si5351_obecny)
        return;
    ...
    si5351_write(SI5351_CRYSTAL_LOAD, ((CFG_GetParam(CFG_PARAM_SI5351_CAPS) & 3) << 6) | 0x12);

    //Set input source
    si5351_write(SI5351_PLL_INPUT_SOURCE, 0);
...
void si5351_OffMeasurement(void)
{
    if (!si5351_obecny)
        return;

    /*
     * CLK0 i CLK1 tworza tor pomiarowy analizatora. Wylaczamy je razem,
     * ale nie dotykamy CLK2, bo ten moze w tej samej chwili pracowac jako
     * niezalezne wyjscie GEN OUT.
     */
...
void si5351_OffF2(void)
{
    if (!si5351_obecny)
        return;

    /* CLK2 jest osobnym torem GEN OUT / S2. */
...
{
    CAMERA_IO_WriteBulk(si5351_bus_addr_runtime, addr, data, (uint16_t)bytes);
    return 0;
}

static uint8_t si5351_write(uint8_t addr, uint8_t data)
{
    CAMERA_IO_Write(si5351_bus_addr_runtime, addr, data);
    return 0;
}

static uint8_t si5351_read(uint8_t addr, uint8_t *data)
{
    *data = CAMERA_IO_Read(si5351_bus_addr_runtime, addr);
    return 0;
}
...
static uint8_t si5351_address_responds(uint8_t addr)
{
    /* Testujemy ACK, a nie wartosc przypadkowego rejestru. */
    return CAMERA_IO_IsDeviceReady(addr);
}

// Zwraca 0, gdy nie znaleziono Si5351, albo jego 8-bitowy adres I2C.
static uint8_t si5351_detect_address(void)
{
    static const uint8_t znane_adresy[] = {0xC0, 0xC4, 0xCE, 0xDE};
    uint32_t i;

    for (i = 0; i < (sizeof(znane_adresy) / sizeof(znane_adresy[0])); ++i)
    {
        if (si5351_address_responds(znane_adresy[i]))
            return znane_adresy[i];
    }
    return 0;
}

uint8_t si5351_GetBusAddress(void)
{
    return si5351_bus_addr_runtime;
}

uint8_t si5351_IsPresent(void)
{
    return si5351_obecny;
}
...
void si5351_dump_regs(void)
{
    ...
    if (FR_OK != fr)
        return;
    uint32_t bw = 0;
    fr = f_write(&fo, reg_dump, 256, &bw);
    f_close(&fo);
    if (FR_OK != fr || bw != 256U)
        f_unlink("/aa/snapshot/5351reg.bin");
}
#endif
