...
#define ADF4350_FPFD_CEL 100000ul
#define ADF4350_FVCO_MIN 2200000000ull //2.2 GHz
#define ADF4350_FVCO_MAX 4400000000ull //4.4 GHz

extern void Sleep(uint32_t);

static uint32_t adf4350_ref_clk(void)
{
    uint32_t f = CFG_GetParam(CFG_PARAM_ADF_REF_FREQ);
    return (f >= 1000000U && f <= 100000000U) ? f : 27000000U;
}

static uint32_t adf4350_r_value(void)
{
    uint32_t r = (adf4350_ref_clk() + ADF4350_FPFD_CEL / 2U) / ADF4350_FPFD_CEL;
    if (r < 1U) r = 1U;
    if (r > 1023U) r = 1023U;
    return r;
}

static uint32_t adf4350_fpfd(void)
{
    return adf4350_ref_clk() / adf4350_r_value();
}
...
void adf4350_Init(void)
{
    ...
    /* Odniesienie moze pochodzic z CLK2 Si5351 albo z zewnetrznego zegara. */
...
uint32_t adf4350_MinFreq(void)
{
    return ADF4350_MIN_OUT_FREQ;
}

uint32_t adf4350_MaxFreq(void)
{
    return ADF4350_MAX_OUT_FREQ;
}
